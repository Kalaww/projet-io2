<?php
	...
	function supprimer(){
		...
		if(isset($_POST['id'])){
			$requete = sprintf("DELETE FROM reunions WHERE id = '%s'",
...
<html>
	<head>
		...
		<meta charset="utf-8">
		<title>Suppression de la réunion</title>
	</head>
	<body>
		<header>
			<?php
		...
		<?php
			if(!isset($_SESSION['login'])){
				...
				echo "<div>Vous n'avez pas l'autorisation de supprimer cette réunion</div>";
			}else if(!$suppression_succes){
				echo "<div>Echec lors de la suppression de la réunion</div>";
			}else{
				echo "<div>La réunion a bien été supprimé</div>";
			}
		?>
